 ...
 * @property double $discount_items
 * @property double $discount_delivery_charge
...
class Order extends \common\models\BaseModel
{
    ...
    {
        return [
            [['restaurant_id', 'postcode', 'delivery_type', 'currency_code', 'delivery_charge', 'subtotal', 'total', 'restaurant_subtotal', 'restaurant_total'], 'required'],
    ...
    {
        return $this->hasOne(RestaurantSchedule::className(), ['restaurant_id' => 'id'])->via('restaurant');
    }
     ...
     * @return \yii\db\ActiveQuery
     */
    public function getLastOrderContactHistory()
    {
        return $this->hasOne(OrderContactHistory::className(), ['order_id' => 'id']);
    }
    ...
    /**
     * Get orders that is not confirmed / cancelled / delivered
     *
     * @return array
     */
    public function getUnstagedOrders()
    {
        $res = self::find()
            ->with([
                'restaurantContacts' => function ($query) {
                    $query->andWhere('record_type = "' . RecordType::Active . '"');
                    $query->andWhere('type = "' . RestaurantContactOrderType::Ivr . '"');
                },
                'restaurantDelivery' => function ($query) {
                    $query->andWhere('record_type <> "' . RecordType::Deleted . '"');
                },
                'orderContactHistories' => function ($query) {
                    $query->andWhere('record_type <> "' . RecordType::Deleted . '"');
                },
                'orderHistories' => function ($query) {
                    $query->andWhere('record_type <> "' . RecordType::Deleted . '"');
                    $query->andWhere('status = "' . OrderStatus::ReadyBy . '"');
                    $query->orderBy('id DESC');
                },
                'lastOrderContactHistory' => function ($query) {
                    $query->andWhere('record_type <> "' . RecordType::Deleted . '"');
                    $query->andWhere('type = "' . RestaurantContactOrderType::Ivr . '"');
                    $query->orderBy('id DESC');
                }
            ])
            ->leftJoin('restaurant', 'restaurant.record_type <> "' . RecordType::Deleted . '" AND restaurant.id = order.restaurant_id')
            ->leftJoin('restaurant_schedule', 'restaurant_schedule.record_type <> "' . RecordType::Deleted . '" AND restaurant_schedule.restaurant_id = restaurant.id')
            ->where('order.status = "' . OrderStatus::ReadyBy . '"')
            ->orWhere('order.delivery_provider = "' . \common\enums\DeliveryProvider::Restaurant . '"')
            ->andWhere('order.record_type <> "' . RecordType::Deleted . '"')
            ->andWhere('day = "' . date('l', time()) . '"')
            ->andWhere('type = "' . \common\enums\RestaurantScheduleType::OpenTime . '"')
            ->andWhere('`from` <= "' . date('H:i:s', time()) . '"')
            ->andWhere('`to` >= "' . date('H:i:s', time()) . '"')
            ->all();
    ...
    {
        $res = self::find()
            ->leftJoin('order_contact_history', 'order_contact_history.order_id = order.id AND order_contact_history.type = "' . RestaurantContactOrderType::Sms .'"')
            ->where('order.status = "' . OrderStatus::OrderCancelled . '"')
            ->andWhere('order.record_type <> "' . RecordType::Deleted . '"')
            ->andWhere('order_contact_history.is_succeeded=0')
            ->all();
    ...
    public function getUnsentEmails()
    {
        ...
        $customerRes = self::find()
            ->select('order.*,order_contact_history.order_status AS status')
            ->leftJoin('order_contact_history', 'order_contact_history.order_id = order.id AND order_contact_history.type = "' . RestaurantContactOrderType::Email .'"')
            ->leftJoin('user', 'user.id = order.user_id')
            ->where('order.status IN (' . $customerGroups . ')')
            ->andWhere('order.record_type <> "' . RecordType::Deleted . '"')
            ->andWhere('order_contact_history.is_succeeded=0')
            ->all();
